...
var List = bookshelf.Model.extend({
  ...
  company_reason: function() {
    return this.belongsToMany(Company, 'list_investor_reason', 'list_id', 'company_id').withPivot(['company_id']);
  },
  market_reason: function() {
    return this.belongsToMany(Market, 'list_investor_reason', 'list_id', 'market_id').withPivot(['market_id']);
  }
  // ,
  // reasons: function(){
