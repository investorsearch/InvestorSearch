...
var done = function(err) {
  console.log('a startup has finished being saved');
  console.log(err);
};


//Given a startup, save it, save its markets, save its links to markets
var saveStartupsMarkets = function(startup, done){

  //arr to store each market's local ID (for foreign key linking)
  var startupMarketsSqlIds = [];

  //save the startup
  updateDB.addNewCompany(startup.id, startup.name, function(startupData) {
    var markets = startup.markets;

    //for each of the startup's markets, find if it already exists in the DB
    var promiseArr = markets.map(function(market){
      return new Promise(function(resolve, reject) {
        updateDB.findMarketByAngelListID(market.id, function(dbMarket){

          //if market does not exist, save it to db
          if (dbMarket === null) {
            updateDB.addNewMarket(market.id, market.name, function(newMarket) {
              startupMarketsSqlIds.push(newMarket.attributes.id);
              resolve();
            });

          //else, just store local ID
          } else {
            startupMarketsSqlIds.push(dbMarket.attributes.id);
            resolve();
          }
        });
      });
    ...
    //once all markets have been saved, save market-company linkes
    Promise.all(promiseArr).then(function() {
      startupMarketsSqlIds.forEach(function(marketId) {
        updateDB.linkMarketAndCompany(marketId, startupData.attributes.id);
      });
      done();
    });
  });
...
async.eachSeries(sample_startup_info, saveStartupsMarkets);
