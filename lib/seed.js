...
sample_startup_info.forEach(function(startup){
  updateDB.addNewCompany(startup.id, startup.name, function() {
    var markets = startup.markets;
    var promiseArr = markets.map(function(market){
      return new Promise(function(resolve, reject) {
        if (allMarketIds.indexOf(market.id) === -1 ) {
          allMarketIds.push(market.id);
          updateDB.addNewMarket(market.id, market.name, function() {
            resolve();
          });
        }
      });
    ...
    Promise.all(promiseArr).then(function() {
      markets.forEach(function(market) {
        updateDB.linkMarketAndCompany(market.id, startup.id);
      });
    });
  });
