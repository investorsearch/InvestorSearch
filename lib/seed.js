...
var current_startpoint = 902;
var current_endpoint = 1000;

var MarketsToSave = marketData.slice(current_startpoint, current_endpoint+1);

MarketsToSave.forEach(function(market) {

  //check to see if market has investor links saved locally- only seed from angel list if no local links exist
  updateDB.checkMarketLink(market.id)
  .then(function(foundInvestors) {
