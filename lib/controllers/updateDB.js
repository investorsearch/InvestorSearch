...
var Company = require('../models/angelData').Company,
    ...
    Location = require('../models/angelData').Location,
    ...
    Location_Investor = require('../models/angelData').Location_Investor,
...
exports.addNewInvestor = function(investor){
  return new Promise(function(resolve, reject){
    ...
    var angellist_url = investor.angellist_url;
    ...
    // }
    return Investor
      .forge({name: name, al_id: al_id, type: type, image: image, linkedin_url: linkedin_url, bio: bio, angellist_url: angellist_url})
      .save()
      .then(function(model){
      ...
      .catch(function(err){
        console.log('error SAVING investor....');
        console.error(err);
      });
  });
};
...
// find an investor
exports.investorExists = function(angellistId, cb) {
  // return new Promise(function(resolve, reject) {
    return Investor
      .where({al_id: angellistId})
      .fetch()
...
exports.saveLocation = function(location, cb) {
  var angelListId = location.id;
...
exports.findLocationByAngelListId = function(angelListId, cb){
  Location
    .where({al_id: angelListId})
    .fetch()
    .then(function(company){
...
exports.linkInvestorAndLocation = function(investorId, locationId) {
  Location_Investor
  .forge({investor_id: investorId, location_id: locationId})
    .save()
    .then(function(model){
      console.log("investor-location link saved");
    })
    .catch(function(err) {
      console.log(err);
    });
