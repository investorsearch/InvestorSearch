...
var findCompanysInvestors = function(companyID, sendBack) {
  ...
  var dataFilter = function(allData) {
    ...
    // Investor tag object does not contain linkedin_url-- need to get..
    // 1st check for investor in local DB, if exists, add linked_in URL to each investor in investortags. If investor NOT saved locally, call angellist to find linkedin URL.

    var findInvestorLinkedIn = investorTags.map(function(investor) {
      return new Promise(function(resolve, reject) {
        updateDB.investorExists(investor.id)
        .then(function(foundInvestor) {
          if (foundInvestor === null) {
            if (investor.type === "User") {
              request('https://api.angel.co/1/users/' + investor.id, function(err, response, body) {
                  var userData = JSON.parse(body);
                  console.log('this is the USERDATA');
                  console.log(userData);
                  investor.linkedin_url = userData.linkedin_url;
                  console.log('investor after url updated from angellist');
                  console.log(investor);
                  resolve(investor);
                });
            } else {
              request('https://api.angel.co/1/startups/' + investor.id, function(err, response, body) {
                  var investmentCoData = JSON.parse(body);
                  console.log('this is the investmentCoData');
                  console.log(investmentCoData);
                  investor.linkedin_url = investmentCoData.linkedin_url;
                  console.log('investmentCoData after url updated from angellist');
                  console.log(investor);
                  resolve(investor);
                });
            }
          } else {
            investor.linkedin_url = foundInvestor.attributes.linkedin_url;
            console.log('investor after url updated from DB');
            console.log(investor);
            resolve(investor);
          }
        });
    ...
    Promise.all(findInvestorLinkedIn).then(function(investorsWithLinkedIn) {
      console.log('sending back investors now.');
      sendBack(investorsWithLinkedIn);
    });
  };
  ...
  request(apiUrl, function(error, response, body) {
    ...
    loopThroughPages(data.page, data.last_page, apiUrl, dataKey, dataSet).then(function(allData) {
      console.log('all pages are finished, sending back data now');
      dataFilter(allData);
    });
  });
};
