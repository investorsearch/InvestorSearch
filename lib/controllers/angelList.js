...
var loopThroughPages = function(currentPage, lastPage, apiUrl, dataKey, dataSet, saveFunc) {
  ...
  //if on last page or api response is only 1 page, execute response callback (poss filter results, save, and respond to request)
    if (currentPage > lastPage) {
      ...
      request(paginatedApiUrl, function(err, response, body) {
        ...
        resolve(loopThroughPages(currentPage, lastPage, apiUrl, dataKey, dataSet, saveFunc));
      });
    }
  });
};
...
var findMarketsCompanys = function(marketID, saveFunc) {
  ...
  request(apiUrl, function(error, response, body) {
    ...
    //get all data if results are paginated
    loopThroughPages(data.page, 100, apiUrl, dataKey, dataSet, saveFunc).then(function(allData) {
...
module.exports = {findMarketsCompanys: findMarketsCompanys};