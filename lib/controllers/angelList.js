...
var findMarketsCompanys = function(marketID) {
  var apiUrl = 'https://api.angel.co/1/tags/' + marketID + '/startups/';

  var dataSet = [];

  var saveData = function(allData) {
    //allData is array of objects, each is a startup. Save the following data to the startup collection:
    //startup.id
    //startup.name
    //startup.markets (arr)
      //market collection id
    //save each market to Markets collection(check doesn't exist already)
      //market.angelListId
      //market.display_name
  };
