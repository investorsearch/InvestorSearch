    ...
    return new Promise(function(resolve, reject) {
      angelList.findCompanysInvestors(company.al_id, function(companyInvestors) {
        ...
        companyInvestors.forEach(function(companyInvestor) {
          ...
          updateDB.investorExists(companyInvestor.id)
          .then(function(investor) {
            ...
            if (investor === null) {
              ...
              updateDB.checkInvestorCompanyLink(investor.attributes.id, company.id)
              .then(function(link){
                if (link === null) {
                  updateDB.linkInvestorAndCompany(investor.attributes.id, company.id);
                }
              });
            }
          });
