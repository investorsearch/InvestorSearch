...
var fake_people =
 ...
 // we want to extract the 'person/' from the path
  ...
  request('http://api.crunchbase.com/v/2/person/'+permalink+'?user_key='+API_KEY, function (error, response, body) {
    if(error){
      console.log("error", error);
    } else{
      ...
      for (var i =0; i < parsedBody.relationships.investments.items.length; i++){
        console.log("--" + parsedBody.relationships.investments.items[i].invested_in.name);
      }
      console.log("Total Number of Investments: " + parsedBody.relationships.investments.paging.total_items);
      res.send(200, parsedBody);
      //the imporant info we will want to extract from here are:
        // Companies they invested in: "relationships" --> "investments" -> "items" -> "invested_in" --> "name"
        // Total Investments: "relationships" -->  "investments" --> "paging" --> "total_items"

      //IMPORTANT NOTE: Not sure why # of investments != the number of company names the api returns
    }
  });
