  ...
  request('http://api.crunchbase.com/v/2/people?user_key='+API_KEY+'&page=1', function (error, response, body) {
      if(error){
        ...
        loopPages(rangeOfPages, "people", "items").then(function(promise){
            ...
            console.log("all people: " + allPeople.length)
...
var fake_people =
  ...
  for (var i = 0; i < fake_people.length; i++){
    if(fake_people[i].name.toLowerCase().replace(/ +?/g, '') === req.params.name){
...
var fake_comp =
  ...
  for (var i = 0; i < fake_comp.length; i++){
    if(fake_comp[i].name.toLowerCase().replace(/ +?/g, '') === req.params.company){
      console.log("found match!");
      match = fake_comp[i];
      //return res.send(200, match);
    }
...
http://api.crunchbase.com/v/2/organization/omgpop/funding_rounds


exports.funding_rounds = function(req, res) {
...
var fake_comp =
 ...
 // we want to extract the 'organization/' from the path
  var permalink = match.path.replace(/^.+\//,'');
  console.log(permalink);
  ...
  request('http://api.crunchbase.com/v/2/organization/'+permalink+'/funding_rounds?user_key='+API_KEY, function (error, response, body) {
    if(error){
      console.log("error", error);
    } else{

      var fundingRounds = JSON.parse(body).data.items;
      console.log("fundingRounds");
      console.log(fundingRounds);

      res.send(200, fundingRounds);
    }
  });

 };

// Once we return an array with a list of funding rounds, we need to loop through the array and look up each funding round.


exports.funding_round = function(req, res) {
// Hit this route with a UUID

  request('http://api.crunchbase.com/v/2/funding-round/'+req.params.id+'?user_key='+API_KEY, function (error, response, body) {
    if(error){
      console.log("error", error);
    } else{

      var investors = JSON.parse(body).data.relationships.investments.items;

      // here we will want to put in logic to look for the correct investor. If we find that investor as a match, then we return the full list of investors for this funding round.

      // this is hard coded to marc andreessen until we get our DB setup.

      for (var i = 0; i < investors.length; i++){
        if(investors[i].investor.path.replace(/^.+\//,'') === 'marc-andreessen') {
          console.log(investors[i]);
          res.send(200, investors);
        }
      }
      res.send(200);
